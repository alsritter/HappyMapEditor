 ...
 * @param item 绘制网格的参数
 */
export function drawGrid(ctx: CanvasRenderingContext2D, width: number, height: number, size: number, initX: number, initY: number): void {
  // 要绘制网格必定要刷新画布
...
export function drawAllItem(
  ...
  for (const tile of tiles) {
    switch (tile.layer) {
      case Layer.FRONT:
        drawSingleItem(frontCtx, width, height, size, initX, initY, tile.point.x, tile.point.y, tile);
        break;
      case Layer.MIDDLE:
        drawSingleItem(middleCtx, width, height, size, initX, initY, tile.point.x, tile.point.y, tile);
        break;
      case Layer.BACKGROUND:
        drawSingleItem(backgroundCtx, width, height, size, initX, initY, tile.point.x, tile.point.y, tile);
        break;
    }
  }
}
